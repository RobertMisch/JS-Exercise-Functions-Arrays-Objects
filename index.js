...
// console.log(makePersonObject(5, "Leia", "leina@leia"));
/**
 * ### Challenge `getName`
 * 
 * @instructions
...
*/
function getName(ourObject) {
  return "Hello, my name is " + ourObject.name + ".";
}
...
function getLastCarInfo(inventory) {
  let ourLastItemIndex = inventory.length -1;
  return "This is a " + inventory[ourLastItemIndex].car_make + " " +inventory[ourLastItemIndex].car_model +".";
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventory, ourid) {
  for(let i=0; i< inventory.length; i++){
    if(inventory[i].id === ourid){
...
*/
function carMaker(inputOdometer) {
  const ourCar = {
    odometer:inputOdometer, 
    drive: function(ourDist){
      ourCar.odometer= ourCar.odometer + ourDist;
      return ourCar.odometer;
    }
  }
  return ourCar;
}
